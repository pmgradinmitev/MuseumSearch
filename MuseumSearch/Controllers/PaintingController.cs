...
using MuseumSearch.Data;
using MuseumSearch.Data.Entities;
using MuseumSearch.ViewModels;
...
namespace MuseumSearch.Controllers
{
    public class PaintingController : Controller
    {
        ...
        private IQueryable<Painting> Search(PaintingsTableViewModel searchModel, IQueryable<Painting> query)
        {
            if (!String.IsNullOrEmpty(searchModel.Name))
                query = query.Where(s => s.Name!.ToUpper().Contains(searchModel.Name.ToUpper()));
            if (!String.IsNullOrEmpty(searchModel.Author))
                query = query.Where(s => s.Author!.ToUpper().Contains(searchModel.Author.ToUpper()));
            return query;
        }
    }
}
